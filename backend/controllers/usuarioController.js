...
function estadoUsuario(req, res) {
    let idusuario = req.params.id
    let activo = req.params.activo
    Usuario.findByIdAndUpdate(idusuario, activo, (err, activoUpdated)=>
    {
        if(err) return res.status(500).send({message: 'Error al realizar la peticion'})
        if(!activoUpdated) return res.status(404).send({message: 'Error, el usuario no existe'})

        res.status(200).send({activo: activoUpdated})
    })
}
...
function listarUno(req, res) {
    ...
    Usuario.findById(id, (err, usuario)=>{
        if (err) return res.status(500).send({message: 'error al realizar la peticion'})
        if(!usuario) return res.status(404).send({message:'Error, el usuario no existe'})
